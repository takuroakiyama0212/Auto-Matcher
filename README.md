...
See [GOOGLE_AUTH_SETUP.md](./GOOGLE_AUTH_SETUP.md) for detailed Firebase and Google OAuth configuration instructions.
...
This generates the `dist/` directory with all static files needed for deployment.
...
2. **Configure Settings** (auto-detected from `vercel.json`):
...
4. **URL Structure**
   - Root URL (`/`) → Demo page with QR code
   - `/app` → Swipe app
...
3. **Access**
   - After deployment: `https://yourusername.github.io/Auto-Matcher/`
...
- `app.json`, `tsconfig.json`, `babel.config.js` - Configuration
- `client/` - All source code
- `assets/` - All images and assets
- `demo.html`, `demo.css`, `demo.js` - Demo page (optional)
...
# Commit
git commit -m "Add all files for deployment"
...
- Check Vercel Dashboard → Settings → Root Directory is empty

**Build fails with "Command exited with 1"**
- Verify `package.json` has `build:web` script
- Check all dependencies are listed in `package.json`
- Ensure Node.js version matches `.nvmrc` (20)
- Test locally: `npm ci && npm run build:web`

**Assets not loading**
- Verify `assets/` directory is committed to GitHub
- Check image paths in `client/data/cars.ts` use `require()`
...
- **`client/screens/`** - Screen components:
  - `BrowseScreen.tsx` - Swipe interface
  - `ProfileScreen.tsx` - User profile and sign-in
  - `FavoritesScreen.tsx` - Saved cars list
  - `CarDetailModal.tsx` - Car details with image gallery
- **`client/components/`** - Reusable components
- **`client/data/cars.ts`** - Car data with image paths
- **`client/lib/firebase.ts`** - Firebase configuration

### Build Output (Auto-generated, Don't Edit)

When you run `npm run build:web`, Expo generates:

```
dist/
├── index.html          # Main HTML entry point (auto-generated)
├── _expo/static/js/    # Compiled JavaScript bundles
└── assets/             # Copied assets
```

**Important**: 
- ⚠️ Do NOT edit files in `dist/` - they are auto-generated
- ✅ Always edit source files in `client/` directory
- ✅ Rebuild after making changes: `npm run build:web`
